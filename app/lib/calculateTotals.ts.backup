...
export function calculateTotals(p: CalcParams): DetailedCalcResult {
  ...
  // 1. Boat Base Price
  const baseAgent = Number(p.selectedBoat.agent_price) || Number(p.selectedBoat.base_price) || 0;
  const baseClient = Number(p.selectedBoat.client_price) || Number(p.selectedBoat.base_price) || 0;
